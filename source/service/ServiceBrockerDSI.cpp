...
IServiceBrockerDsiPtr ServiceBrockerDSI::mp_instance;
...
IServiceBrockerDsiPtr ServiceBrockerDSI::instance( )
{
   os::Mutex mutex( true );
   if( !mp_instance )
   {
...
void ServiceBrockerDSI::thread_loop_send( )
{
   ...
   while( started_send( ) )
   {
      ...
      SYS_MSG( "send(%d): %d bytes", m_master_socket, send_size );
   }
...
void ServiceBrockerDSI::thread_loop_receive( )
{
   ...
   while( started_receive( ) )
   {
      ...
      SYS_MSG( "recv(%d): %d bytes", m_master_socket, recv_size );
