...
void* Thread::thread_loop( void* parameters )
{
   ...
   SYS_INF( "Thread: %#x. Enter", id );
   if( id != p_thread->m_id )
   {
      SYS_ERR( "Thread ID mistmach: %#x != %#x", id, p_thread->m_id );
      mutex.unlock( );
...
bool Thread::run( )
{
   if( true == m_created )
   {
      SYS_WRN( "Thread: %#x. Already created", m_id );
      return false;
   }
      ...
      SYS_INF( "Thread: %#x. Created", convert( m_id ) );
      m_created = true;
   #endif
...
bool Thread::join( )
{
   if( false == m_created ) 
   {
      ...
      if( 0 == result )
      {
         SYS_INF( "Thread: %#x. Joined with status: %p", m_id, ((int**)status) );
         m_created = false;
      }
      else
      {
         SYS_ERR( "Thread: %#x. Was not joined. Error: %d", m_id, result );
      }
   #elif OS == STD
