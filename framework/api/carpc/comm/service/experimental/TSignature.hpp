...
namespace carpc::service::experimental {
   ...
   class TSignature
   {
      ...
      public:
         TSignature( ) = default;
         TSignature(
               const comm::service::Name&, const _ID&, const eType&,
   ...
   TSignature< _ID >::TSignature(
               const comm::service::Name& role, const _ID& id, const eType& type,
               const comm::service::ID& from, const comm::service::ID to,
               const comm::sequence::ID seq_id
   )
      : m_role( role )
   ...
   {
      return ipc::serialize( stream, m_role, m_id, m_type, m_from, m_to, m_seq_id );
   }

   template< typename _ID >
   const bool TSignature< _ID >::from_stream( ipc::tStream& stream )
   {
      return ipc::deserialize( stream, m_role, m_id, m_type, m_from, m_to, m_seq_id );
   }
   ...
   template< typename _ID >
   const eType& TSignature< _ID >::type( ) const
   {
      return m_type;
   }
