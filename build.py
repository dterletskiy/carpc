...
def extend_configuration( ):
   ...
   environment: dict = pfw.os.build_environment( None, env_overwrite, env_add )
...
def start_target( **kwargs ):
   ...
   pid: int = pfw.os.is_process_running( kw_target )
   if None != pid:
...
def start_dlt_daemon( **kwargs ):
   ...
   pid: int = pfw.os.is_process_running( kw_dlt_daemon )
   if None != pid:
...
def stop_dlt_daemon( **kwargs ):
   ...
   pfw.os.kill_process( kw_dlt_daemon )
# def stop_dlt_daemon
