...
namespace carpc::async {
   ...
   class TPassport : public IEvent::ISignature
   {
      ...
      public:
         TPassport( ) = default;
         TPassport( const tUserSignature& user_signature )
            : m_user_signature( user_signature )
         { }
         TPassport( const TPassport& other ) = delete;
         ~TPassport( ) override = default;
   ...
   // template< typename _Generator >
   // const tAsyncTypeID
   //    TPassport< _Generator >::ms_type_id = build_type_id( );
